...
Cookies reuse your browser's signed-in session so VidDownloadPro can download content that requires login or verification, such as subscriber-only items, age-gated pages, or private links.
...
## Two ways to use cookies in VidDownloadPro

### Option 1: Read cookies from your browser

In **Settings**, select your browser. VidDownloadPro will try to detect the browser profile path automatically. You can also enter the profile path manually.
...
1. Open VidDownloadPro → Settings → Cookies.
...
2. Open VidDownloadPro → Settings → Cookies file, and select the exported file.
