---
title: FAQ
description: Common questions and troubleshooting for VidDownloadPro
---

## What if a download fails or shows an error?

- Confirm the link is valid and opens in your browser.
- Update VidDownloadPro to the latest version and retry.
...
- Make sure VidDownloadPro is up to date.
- Reduce the number of concurrent downloads.
- Use cookies to reuse your signed-in session when needed.

## Which sites are supported?

VidDownloadPro uses the yt-dlp extractor system and supports many sites. Try the download first; if it fails, submit feedback with the link and error details.
...
```bash
xattr -rd com.apple.quarantine /Applications/VidDownloadPro.app/
```
